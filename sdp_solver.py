...
def makeSparse(mat):
    """ makes input numpy.array a fusion.SparseMatrix object """
    titi=sparse.coo_matrix(np.around(mat,8))
...
def main(red,n):
    ...
    PTproR=makeSparse(np.real(DMany([PT,pro,np.conj(toBloch(5)).T])))
    PTproI=makeSparse(np.imag(DMany([PT,pro,np.conj(toBloch(5)).T])))
    ...
    with Model("Negativity") as M:
        ...
        H1=M.variable("H1",Domain.inPSDCone(2**n))
        H2=M.variable("H2",Domain.inPSDCone(4))
        H3=M.variable("H3",Domain.inPSDCone(4))
        ...
        cPSDall=M.constraint(Expr.sub(RePall,H1),Domain.equalsTo(0.))
        ...
        cPSDa=M.constraint(Expr.sub(RePa,H2),Domain.equalsTo(0.))
        ...
        cPSDb=M.constraint(Expr.sub(Expr.mul(-1.,RePb),H3),Domain.equalsTo(0.))
        ...
        M.objective(ObjectiveSense.Minimize,t.asExpr())
        M.solve()
        return t.level()
